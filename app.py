...
gym_df = pd.read_csv('db/Gym.csv')
gym_pokemon_df = pd.read_csv('db/Gym_Pokemon.csv')

# read pokemon info and pokemon type
conn = sqlite3.connect('db/database_1214.db')
with open('db/pokemon.sql', 'r', encoding='utf-8') as file:
    sql_query = file.read()

raw_df = pd.read_sql_query(sql_query, conn)
raw_df = raw_df.loc[:, ~raw_df.columns.duplicated()]

# Group by unique identifiers and aggregate types
def separate_types(group):
    types = list(group['type'].unique())  # Get unique types
    return pd.Series({
        'HP': group['HP'].iloc[0],
        'attack': group['attack'].iloc[0],
        'defense': group['defense'].iloc[0],
        'special_attack': group['special_attack'].iloc[0],
        'special_defense': group['special_defense'].iloc[0],
        'speed': group['speed'].iloc[0],
        'type1': types[0] if len(types) > 0 else None,
        'type2': types[1] if len(types) > 1 else None  # NULL if only one type
    })
group_columns = raw_df[['pokemon_generation', 'name']]
df = (
    raw_df.groupby(['pokemon_generation', 'name'])
    .apply(separate_types)
    .reset_index(drop=True)
)

df = pd.concat([group_columns, df], axis=1)


# merge all tables excluding effectiveness tables
...
merged_df['leader_image_path'] = merged_df.apply(lambda row: f"assets/gen1_leaders/{row['leader'].lower()}.png", axis=1)
...
raw_eff = pd.read_csv('db/Type_Effectiveness.csv')
types = raw_eff['type'].tolist()  
type_effectiveness = raw_eff.drop(columns=['type']).to_numpy()

# bar chart
result_df = merged_df[['gym_id', 'generation', 'pokemon', 'leader', 'type1', 'type2']]

color_list = ['#FFE5B4', '#F8B195', '#FFB700','#2FBDBD','#80DAEB', '#6798C0']
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div("Use hovertext to build your team to counter the gym leader's most common type.",
