...
pokemon_df = pd.read_csv('db/Pokemon_Info.csv')

# read pokemon info and pokemon type
type_df = pd.read_csv('db/Pokemon_Types.csv')
type_df['type_num'] = type_df.groupby('name').cumcount() + 1
df_pivot = type_df.pivot_table(index='name', columns='type_num', values='type', aggfunc='first')
df_pivot.columns = [f'type{i}' for i in df_pivot.columns]
df = pd.merge(df_pivot, pokemon_df, on='name')
